...
@Component("TitleCrawlingMapper")
public class TitleCrawlingMapper implements ITitleCrawlingMapper {
	...
	public boolean createCollection(String colNm) throws Exception {
		...
		mongodb.createCollection(colNm).createIndex(new BasicDBObject("collect_time", 1).append("Title", 1), "Title");
	...
	public List<TitleDTO> getTitle(String colNm) throws Exception {
		...
		log.info(this.getClass().getName() + ".뉴스타이틀 셀렉트 매퍼 시작");
		...
		while (cursor.hasNext()) {
			...
			String Crawling_Data = CmmUtil.nvl((String) current.get("title")); // 크롤링데이터
		...
		log.info(this.getClass().getName() + ".뉴스타이틀 셀렉트 매퍼 종료");
